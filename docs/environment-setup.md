...
- WhatsApp Business: `WHATSAPP_CONFIG_ID` in the agent, which is the Embedded Signup configuration id of the same Meta app. With it set, a merchant connects WhatsApp by authorizing once in Meta's own dialog; the agent exchanges the returned code, subscribes the WhatsApp Business Account to webhooks, and registers the number itself. `WHATSAPP_APP_ID`, `WHATSAPP_APP_SECRET`, and `WHATSAPP_VERIFY_TOKEN` are optional overrides that each fall back to the `FB_` equivalent, and `META_GRAPH_API_VERSION` falls back to `FB_GRAPH_API_VERSION`. Without `WHATSAPP_CONFIG_ID` only the advanced path remains, where a merchant pastes a phone number ID and a permanent token.
