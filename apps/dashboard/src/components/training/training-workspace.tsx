...
import { AlertTriangle, BookOpen, Bot, Check, ChevronDown, ChevronRight, FileUp, Globe2, Loader2, Package, RefreshCw, Sparkles, Trash2 } from 'lucide-react';
...
function CandidateCard({ candidate, refresh, selected, onSelect }: { candidate: TrainingCandidate; refresh: () => void; selected?: boolean; onSelect?: (checked: boolean) => void }) {
    ...
    const [currency, setCurrency] = useState(String(candidate.payload.currency || ''));
    const [stock, setStock] = useState(candidate.payload.stock === null || candidate.payload.stock === undefined ? '' : String(candidate.payload.stock));
    const [productAvailability, setProductAvailability] = useState(String(candidate.payload.availability || 'unknown'));
    ...
    const action = useMutation({ mutationFn: async (kind: string) => {
        ...
        const payload: Record<string, unknown> = candidate.kind === 'product' || candidate.kind === 'offering'
            ? { name, description: content, images, currency, availability: productAvailability, stock: stock === '' ? null : Number(stock), ...(price ? { basePrice: Number(price) } : {}) }
            : { title: name, content };
    ...
    return <div className={`rounded-2xl border p-4 ${tone}`}>
        ...
        {editing && <div className="mt-4 grid gap-3 rounded-xl border bg-background p-3"><Input value={name} onChange={(event) => setName(event.target.value)} placeholder="Name or title"/>{candidate.kind === 'product' && <><div className="grid gap-3 sm:grid-cols-3"><Input type="number" value={price} onChange={(event) => setPrice(event.target.value)} placeholder="Price"/><select className="rounded-md border bg-background px-3" value={currency} onChange={(event) => setCurrency(event.target.value)}><option value="">Choose currency</option>{['BDT','USD','EUR','GBP','INR'].map((code) => <option key={code} value={code}>{code}</option>)}</select><select className="rounded-md border bg-background px-3" value={productAvailability} onChange={(event) => setProductAvailability(event.target.value)}><option value="in_stock">In Stock</option><option value="out_of_stock">Out of Stock</option><option value="preorder">Preorder</option><option value="unknown">Stock unknown</option></select></div><Input type="number" min="0" value={stock} onChange={(event) => setStock(event.target.value)} placeholder="Exact quantity (leave blank if unknown)"/><ImageUpload value={images} onChange={setImages} disabled={action.isPending} folder="training-review" maxFiles={12}/></>}<textarea className="min-h-24 rounded-md border bg-background p-3 text-sm" value={content} onChange={(event) => setContent(event.target.value)} placeholder="Description or content"/><Button className="w-fit" size="sm" disabled={action.isPending || (candidate.kind === 'product' && (!price || !currency))} onClick={() => action.mutate('edit')}>Save changes</Button></div>}
    </div>;
}
...
export function TrainingWorkspace({ onboarding = false, onFinish }: { onboarding?: boolean; onFinish?: () => Promise<void> }) {
    ...
    const pendingBusinessType = useRef('');
    ...
    useEffect(() => { if (statusQuery.data?.businessProfile) { const savedType = statusQuery.data.businessProfile.businessType || ''; if (!pendingBusinessType.current || pendingBusinessType.current === savedType) { pendingBusinessType.current = ''; setBusinessType(savedType); } setBusinessSubType(statusQuery.data.businessProfile.businessSubType || ''); setCustomBusinessType(statusQuery.data.businessProfile.customBusinessType || ''); } }, [statusQuery.data?.businessProfile]);
    ...
    const setupQuestions = overview?.setupQuestions?.[businessType] || [];
    const remainingGaps = overview?.gaps?.filter((gap) => !setupQuestions.some((question) => question.id === gap.id)) || [];
    return <div className="space-y-6">
        ...
        <div className="grid items-start gap-6 xl:grid-cols-[200px_minmax(0,1fr)]">
            <nav aria-label="Training workflow" className="work-panel flex flex-wrap gap-1.5 sm:gap-2 p-2.5 sm:p-3 xl:sticky xl:top-0 xl:flex-col">
                {[
                    ['training-business', '01', 'Your business'],
                    ['training-sources', '02', 'Connected sources'],
                    ['training-review-start', '03', 'Review & approve'],
                    ['training-test', '04', 'Open AI Assistant'],
                ].map(([id,number,title]) => <a key={id} href={`#${id}`} className="flex items-center gap-2 rounded-lg px-2.5 py-2 text-xs sm:text-sm hover:bg-muted"><span className="text-xs text-primary">{number}</span>{title}</a>)}
                <p className="hidden px-3 pb-2 pt-4 text-xs leading-6 text-muted-foreground xl:block">You stay in control. Imported details are staged for your review before approval.</p>
            </nav>
