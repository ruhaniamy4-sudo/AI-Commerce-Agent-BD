    ...
    async function submit(event: React.FormEvent) {
        ...
        try {
            ...
            if (body.emailDeliveryConfigured === false) {
                ...
                setMessage('If verification is available for this account, an email has been sent.');
            }
        } catch (error) {
            setIsError(true);
    ...
    return <main className="flex min-h-screen items-center justify-center bg-slate-50 p-4">
        <Card className="w-full max-w-md">
            <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">Request a new one-time verification link.</p>
            </CardHeader>
            <CardContent>
                {message ? <div className="space-y-4">
                    ...
                    <Link className="font-semibold text-primary" href="/login">Return to sign in</Link>
                </div> : <form onSubmit={submit} className="space-y-4">
                    <Input type="email" placeholder="Email" value={email} onChange={event => setEmail(event.target.value)} required />
                    <Button className="w-full" disabled={busy}>{busy ? 'Sending…' : 'Send verification email'}</Button>
                </form>}
            </CardContent>
        </Card>
    </main>;
