...
export default function ConversationDetailPage() {
  ...
  return <div className="space-y-6">
    <PageHeader title={conversation?.customer?.name || 'Conversation'}
      description={[conversation?.customer?.phone, conversation?.psid].filter(Boolean)[0] || 'Customer history'}
      actions={<div className="flex items-center gap-3"><ChannelBadge channel={conversation?.channel}/><HandlerBadge controlMode={conversation?.controlMode} needsHumanHandoff={conversation?.needsHumanHandoff}/><Button variant="outline" onClick={() => router.push('/conversations')}><ArrowLeft size={16} className="mr-2" />All conversations</Button></div>} />
    <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_380px]"><WorkspacePanel title="Message history" description="Saved customer and assistant messages"
      ...
      : <div role="log" aria-label="Conversation history" className="max-h-[65dvh] space-y-6 overflow-y-auto p-4 sm:p-6">{messages.map((message) => <article key={message._id} className={cn('flex flex-col', message.role === 'user' ? 'items-start' : 'items-end')}>
        ...
        <p className={cn('max-w-[95%] whitespace-pre-wrap wrap-break-word rounded-2xl px-4 py-3 text-sm leading-6 sm:max-w-[80%]',message.role === 'user' ? 'border border-border bg-muted text-foreground' : 'bg-primary text-primary-foreground')}>{message.role === 'user' || message.metadata?.source === 'human' ? message.content : customerFacingText(message.content)}</p>
      </article>)}</div>}
      <footer className="border-t border-border p-4">{conversation?.controlMode === 'HUMAN_ACTIVE' ? <form className="flex gap-2" onSubmit={event => { event.preventDefault(); if (reply.trim()) replyMutation.mutate() }}><input className="h-10 flex-1 rounded-lg border border-border bg-background px-3 text-sm outline-hidden focus:border-primary" value={reply} onChange={event => setReply(event.target.value)} placeholder="Reply as your team…" maxLength={2000}/><Button type="submit" disabled={!reply.trim() || replyMutation.isPending}>{replyMutation.isPending ? <Loader2 size={16} className="animate-spin"/> : <Send size={16}/>}<span className="ml-2">Send</span></Button></form> : <p className="text-center text-xs text-muted-foreground">Take over this conversation to reply manually.</p>}{replyMutation.isError && <p role="alert" className="mt-2 text-xs text-destructive">{(replyMutation.error as Error)?.message || 'The reply could not be delivered. Check the channel connection and try again.'}</p>}</footer>
