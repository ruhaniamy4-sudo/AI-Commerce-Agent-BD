export interface PaymentEvidence {reference:string;orderNumber:string;amount:number;currency:string;status:'pending'|'paid'|'failed'|'refunded';refundedAmount?:number;live:boolean;paymentMethod?:string;}
...
export function sslEvidence(data:any,live:boolean):PaymentEvidence{
 ...
 const method=String(data.card_type||data.card_brand||'').toLowerCase();
 return {reference:String(data.val_id),orderNumber:String(data.tran_id),amount:Number(data.amount),currency:String(data.currency).toUpperCase(),status:'paid',live,paymentMethod:method.includes('bkash')?'bkash':method.includes('nagad')?'nagad':method||'sslcommerz'};
}
