...
describe('business ingestion staging and approval', () => {
    ...
    it('mirrors approved imported product images into the active tenant product', async () => {
        const candidate: any = {
            ...
            payload: { name: 'Imported Polo', description: 'Cotton polo', category: 'Shirts', basePrice: 1490, stock: 5, images: ['https://fabrilife.com/product.jpg'], variants: [], specs: {} },
        ...
        expect(create).toHaveBeenCalledWith(expect.objectContaining({ businessId, images: ['https://res.cloudinary.com/sellpilot/image/upload/imported.jpg'], imageImports: [expect.objectContaining({ status: 'mirrored' })] }));
    });
